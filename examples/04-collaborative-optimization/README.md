...
#### 3️⃣ PC (Sparsity-Cluster-preserving)

```
기본 모델 → 프루닝 → 클러스터링 → Float32
          (50%)    (고유값 감소)
...
- 프루닝과 클러스터링의 상승 효과 학습
- PCQAT로 가기 전 기초 단계
- 정확도와 압축의 균형이 필요한 경우

#### 4️⃣ PCQAT (Sparsity-Cluster-preserving QAT) ⭐
...
PC (Float32):             18.56 KB (67%)
PC (Int8):                9.28 KB (33%)
```

#### 예제 4: PCQAT (04-pcqat.py) ⭐⭐⭐
...
- `mnist_model_pc_f32.tflite` - PC Float32 모델
- `mnist_model_pc_int8.tflite` - PC Int8 모델
...
1. `01-cqat.py` - 클러스터링 기반 (중간 압축, 높은 정확도)
2. `02-pqat.py` - 프루닝 기반 (중간-높은 압축)
3. `03-pc.py` - 프루닝+클러스터링 (중간 압축, 높은 정확도)
4. `04-pcqat.py` - 모든 기법 조합 (최고 압축 ⭐)
...
│  pc_int8        9.28      98.00%   52.10 ms    19.2    │
