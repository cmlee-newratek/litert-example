"""
라즈베리파이 4 벤치마크 스크립트 (Clustering 모델)

클러스터링된 MNIST 모델의 추론 성능을 라즈베리파이 4에서 측정합니다.
...
요구사항:
- TensorFlow 2.13+ 또는 tflite-runtime
- NumPy
"""
...
def load_mnist_data():
    """MNIST 테스트 데이터 로드"""
    data_dir = Path(__file__).parent / "mnist_clustered_models"
...
def main():
    print("=" * 70)
    print("Raspberry Pi 4 벤치마크 - 클러스터링 모델")
    print("=" * 70)
    ...
    # 3. 모델 경로
    print("\n[3] 벤치마크할 모델 검색 중...")
    models_dir = Path(__file__).parent / "mnist_clustered_models"

    models = {
        "baseline": models_dir / "mnist_model_baseline.tflite",
        "clustered_8": models_dir / "mnist_model_clustered_8.tflite",
        "clustered_16": models_dir / "mnist_model_clustered_16.tflite",
        "clustered_32": models_dir / "mnist_model_clustered_32.tflite",
        "clustered_16_quant": models_dir / "mnist_model_clustered_16_quant.tflite",
        "clustered_16_int8": models_dir / "mnist_model_clustered_16_int8.tflite",
    }
    ...
    if results["models"]:
        print("\n📊 벤치마크 결과 요약:")
        print(
            f"    {'모델':<25} {'크기':<12} {'정확도':<12} {'평균 추론':<12} {'FPS':<10}"
        )
        print("    " + "-" * 70)
